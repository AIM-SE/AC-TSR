...
class AttackSASRecTrainer(AbstractTrainer):
    r"""The basic Trainer for basic training and evaluation strategies in recommender systems. This class defines common
    functions for training and evaluation processes of most recommender system models, including fit(), evaluate(),
    resume_checkpoint() and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        super(AttackSASRecTrainer, self).__init__(config, model)
        ...
        self.best_valid_result = None
        self.train_loss_dict = dict()
        # self.attack_optimizer, self.calibrate_optimizer = self._build_optimizer()
        self.optimizer = self._build_optimizer()
        self.eval_type = config['eval_type']
        self.eval_collector = Collector(config)
        self.evaluator = Evaluator(config)
        ...
        self.logger.info('use attack trainer!!!')

    def _build_optimizer_debug(self, **kwargs):
        r"""Init the Optimizer
        ...
        Returns:
            torch.optim: the optimizer
        """
        attack_params = [p for n, p in self.model.named_parameters() if
                         'attack_key_transform' in n or 'attack_query_transform' in n]
        calibrate_params = [p for n, p in self.model.named_parameters() if
                            not ('attack_key_transform' in n or 'attack_query_transform' in n)]
        learner = kwargs.pop('learner', self.learner)
        assert learner.lower() == 'adam'
        ...
        attack_optimizer = optim.Adam(attack_params, lr=learning_rate * 10, weight_decay=weight_decay)
        calibrate_optimizer = optim.Adam(calibrate_params, lr=learning_rate, weight_decay=weight_decay)
        return attack_optimizer, calibrate_optimizer
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False,
        ...
        Returns:
            ...
            tuple which includes the sum of loss in each part.
        """
        assert attack or calibrate
        self.model.train()
        loss_func = loss_func or self.model.calculate_loss
        total_attacked_loss = None
        total_calibrated_loss = None
        iter_data = (
            tqdm(
                train_data,
                total=len(train_data),
        ...
        for batch_idx, interaction in enumerate(iter_data):
            interaction = interaction.to(self.device)
            self.optimizer.zero_grad()
            attacked_loss, calibrated_loss = loss_func(interaction)
            if attacked_loss is not None:
                self._check_nan(attacked_loss)
                total_attacked_loss = attacked_loss.item() if \
                    total_attacked_loss is None else total_attacked_loss + attacked_loss.item()
            self._check_nan(calibrated_loss)
            total_calibrated_loss = calibrated_loss.item() if \
                total_calibrated_loss is None else total_calibrated_loss + calibrated_loss.item()

            for name, p in self.model.named_parameters():
                if 'attack_key_transform' in name or 'attack_query_transform' in name:
                    p.requires_grad = False
                else:
                    p.requires_grad = True
            calibrated_loss.backward(retain_graph=True)
            if attacked_loss is not None:
                for name, p in self.model.named_parameters():
                    if 'attack_key_transform' in name or 'attack_query_transform' in name:
                        p.requires_grad = True
        ...
        if total_attacked_loss is None:
            total_attacked_loss = 0
        return total_attacked_loss, total_calibrated_loss
    ...
    def _save_checkpoint(self, epoch, verbose=True, **kwargs):
        ...
        state = {
            ...
            'state_dict': self.model.state_dict(),
            'other_parameter': self.model.other_parameter(),
            # 'attack_optimizer': self.attack_optimizer.state_dict(),
            # 'calibrate_optimizer': self.calibrate_optimizer.state_dict()
            'optimizer': self.optimizer.state_dict()
        }
        torch.save(state, saved_model_file)
        if verbose:
    ...
    def resume_checkpoint(self, resume_file):
        ...
        # load optimizer state from checkpoint only when optimizer type is not changed
        # self.attack_optimizer.load_state_dict(checkpoint['attack_optimizer'])
        # self.calibrate_optimizer.load_state_dict(checkpoint['calibrate_optimizer'])
        self.optimizer.load_state_dict(checkpoint['optimizer'])
        message_output = 'Checkpoint loaded. Resume training from epoch {}'.format(self.start_epoch)
        self.logger.info(message_output)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        attack_warm_up = 5
        # calibrate_warm_up = 10
        for epoch_idx in range(self.start_epoch, 2 * self.epochs):
            # train
            # if epoch_idx == 0:
            #     self.logger.info('attack warm up!')
            # elif epoch_idx == 5:
            #     self.logger.info('calibrate warm up!')
            training_start_time = time()
            attack = True
            calibrate = True
            # if epoch_idx < attack_warm_up:
            #     calibrate = False
            # elif epoch_idx % 2:
            ...
            #     calibrate = False
            train_attacked_loss, train_calibrated_loss = self._train_epoch(train_data,
                                                                           epoch_idx,
                                                                           show_progress=show_progress,
                                                                           attack=attack,
                                                                           calibrate=calibrate)
            # self.train_loss_dict[epoch_idx] = sum(train_loss) if isinstance(train_loss, tuple) else train_loss
            self.train_loss_dict[epoch_idx] = (train_attacked_loss, train_calibrated_loss)
            training_end_time = time()
            train_attacked_loss_output = \
                self._generate_train_loss_output(epoch_idx, training_start_time, training_end_time,
                                                 train_attacked_loss)
            train_calibrated_loss_output = \
                self._generate_train_loss_output(epoch_idx, training_start_time, training_end_time,
                                                 train_calibrated_loss)
            if verbose:
                self.logger.info(train_attacked_loss_output)
                self.logger.info(train_calibrated_loss_output)
            self._add_train_loss_to_tensorboard(epoch_idx, train_attacked_loss, tag='Train/attacked_loss')
            self._add_train_loss_to_tensorboard(epoch_idx, train_calibrated_loss, tag='Train/calibrated_loss')
            # self.wandblogger.log_metrics({'epoch': epoch_idx, 'train_loss': train_loss, 'train_step': epoch_idx}, head='train')
            self.wandblogger.log_metrics({'epoch': epoch_idx,
                                          'train_attacked_loss': train_attacked_loss,
                                          'train_calibrated_loss': train_calibrated_loss,
            ...
            if self.eval_step <= 0 or not valid_data:
                if saved:
                    self._save_checkpoint(epoch_idx, verbose=verbose)
                continue
            if (epoch_idx + 1) % self.eval_step == 0 and calibrate:
                valid_start_time = time()
                valid_score, valid_result = self._valid_epoch(valid_data, show_progress=show_progress)
                update_flag = False
                stop_flag = False
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        try:
            # Note: interaction without item ids
            _, scores = self.model.full_sort_predict(interaction.to(self.device))
        except NotImplementedError:
            inter_len = len(interaction)
            new_inter = interaction.to(self.device).repeat_interleave(self.tot_item_num)
            batch_size = len(new_inter)
            new_inter.update(self.item_tensor.repeat(inter_len))
            if batch_size <= self.test_batch_size:
                _, scores = self.model.predict(new_inter)
            else:
                _, scores = self._spilt_predict(new_inter, batch_size)
    ...
    def _neg_sample_batch_eval(self, batched_data):
        ...
        batch_size = interaction.length
        if batch_size <= self.test_batch_size:
            _, origin_scores = self.model.predict(interaction.to(self.device))
        else:
            _, origin_scores = self._spilt_predict(interaction, batch_size)
    ...
    def _spilt_predict(self, interaction, batch_size):
        ...
        for i in range(num_block):
            ...
            for key, spilt_tensor in spilt_interaction.items():
                current_interaction[key] = spilt_tensor[i]
            _, result = self.model.predict(Interaction(current_interaction).to(self.device))
            if len(result.shape) == 0:
                result = result.unsqueeze(0)
            result_list.append(result)
        return torch.cat(result_list, dim=0)
...
class AttackRSASRecTrainer(AttackSASRecTrainer):
    def __init__(self, config, model):
        super(AttackRSASRecTrainer, self).__init__(config, model)

class ACSASRecTrainer(AttackSASRecTrainer):
    def __init__(self, config, model):
        super(ACSASRecTrainer, self).__init__(config, model)

class AcBERT4RecTrainer(AttackSASRecTrainer):
    def __init__(self, config, model):
        super(AcBERT4RecTrainer, self).__init__(config, model)
