...
class AcBERT4Rec(SequentialRecommender):

    def __init__(self, config, dataset):
        super(AcBERT4Rec, self).__init__(config, dataset)
    ...
    def _cal_loss(self, seq_output, pos_items, targets):
    ...
    def calculate_loss(self, interaction):
        ...
        attacked_output, calibrated_output, all_attack_masks = self.forward(masked_item_seq)
        ...
        attacked_seq_output = torch.bmm(pred_index_map, attacked_output)  # [B mask_len H]
        calibrated_seq_output = torch.bmm(pred_index_map, calibrated_output)
        # print(attacked_seq_output.shape, calibrated_seq_output.shape)
        # exit('debug')
        targets = (masked_index > 0).float().view(-1)  # [B*mask_len]

        attacked_loss = -self._cal_loss(attacked_seq_output, pos_items, targets)
        mask_penalty = []
        ...
        calibrated_loss = self._cal_loss(calibrated_seq_output, pos_items, targets)
        # print(final_attacked_loss, calibrated_loss)
        # exit('debug')
    ...
    def predict(self, interaction):
        ...
        attacked_output, calibrated_output, _ = self.forward(item_seq)
        attacked_seq_output = self.gather_indexes(attacked_output, item_seq_len)  # [B H]
        calibrated_seq_output = self.gather_indexes(calibrated_output, item_seq_len)
        test_item_emb = self.item_embedding(test_item)

        attacked_scores = torch.mul(attacked_seq_output, test_item_emb).sum(dim=1)  # [B]
        scores = torch.mul(calibrated_seq_output, test_item_emb).sum(dim=1)  # [B]
    ...
    def full_sort_predict(self, interaction):
        ...
        attacked_scores = torch.matmul(attacked_seq_output, test_items_emb.transpose(0, 1))  # [B n_items]
        scores = torch.matmul(calibrated_seq_output, test_items_emb.transpose(0, 1))  # [B n_items]
        return attacked_scores, scores
